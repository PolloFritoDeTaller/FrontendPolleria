    ...
    <>
      {(userRole === 'admin' || userRole === 'client' || userRole === 'worker') && (
        <header className="fixed top-0 left-0 right-0 flex flex-col md:flex-row items-center justify-between p-4 bg-red-600 text-white shadow-lg space-y-2 md:space-y-0 z-50">
          ...
          <div className="flex items-center justify-between w-full md:w-auto relative z-60">
            <button
          ...
          <div className={`flex flex-col md:flex-row items-center space-x-0 md:space-x-4 w-full md:w-auto mt-2 md:mt-0`}>
            ...
            <div className="relative z-60" ref={branchMenuRef}>
              ...
              {showBranches && (
                <div className="absolute left-0 mt-2 w-48 bg-white text-gray-800 shadow-lg rounded-lg z-60">
                  {branches.length > 0 ? (
            ...
            <button
              ...
              className="user-menu-button flex items-center space-x-2 bg-white text-red-600 py-1 px-3 rounded-full shadow-md hover:bg-gray-100 transition-colors z-60"
            >
              <span className="text-lg font-medium">{user.name}</span>
              <FaUser className="text-2xl" />
            </button>
            <div className="absolute top-14 right-1 z-60" ref={userMenuRef}>
              {showUserMenu && (
                <div className="absolute right-0 top-full mt-2 w-48 bg-white text-red-600 shadow-lg rounded-lg z-60">
                  <ul>
                    <li
